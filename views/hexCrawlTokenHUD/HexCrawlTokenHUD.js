...
export const applyHexCrawlHudToHtml = (hud, html) => {
  ...
  $('.col.left', html).prepend(
    $('<div>').addClass("cs-hc-si-phc attribute") //playerHudControl
      .append(timeElement)
      .change(currentTimeField.onchange));

  $('.col.right', html).html(
    $('<div>').addClass("cs-hc-si-phc attribute elevation") //playerHudControl
      .append(movesElement)
      .change(currentMovesField.onchange)
  )
    .append(
      $('<div>').addClass("cs-hc-si-phc control-icon") //playerHudControl
